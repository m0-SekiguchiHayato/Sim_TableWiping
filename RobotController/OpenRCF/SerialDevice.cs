...
namespace OpenRCF
{
    public static class SerialDevice
    {
        ...
        private class Packet
        {
            ...
            public short ToInt16(int startIndex) { return BitConverter.ToInt16(bytes, startIndex); }

            public ushort ToUInt16(int startIndex) { return BitConverter.ToUInt16(bytes, startIndex); }

            public int ToInt32(int startIndex) { return BitConverter.ToInt32(bytes, startIndex); }

            public uint ToUInt32(int startIndex) { return BitConverter.ToUInt32(bytes, startIndex); }

            public float ToSingle(int startIndex) { return BitConverter.ToSingle(bytes, startIndex); }
            ...
            public byte[] Get
            {
                get
                {
                    ...
                    for (int i = 0; i < length; i++) { result[i] = bytes[i]; }
                    return result;
                }
            }
        ...
        public class Arduino : SerialBase
        {
            ...
            public short[] RecieveShortArray { get { return ReceiveData.ShortArray; } }
            ...
            private class ReceiveDataHandler
            {
                ...
                public short[] ShortArray
                {
                    get
                    {
                        ...
                        for (int i = 0; i < result.Length; i++) { result[i] = shortArray[i]; }
                        return result;
                    }
                }
        ...
        public class Dynamixel : SerialBase
        {
            private ReceiveDataHandler ReceiveData = new ReceiveDataHandler();
            ...
            public int Position(byte id) { return ReceiveData.Servo[id].Position; }
            public int Velocity(byte id) { return ReceiveData.Servo[id].Velocity; }
            public int Current(byte id) { return ReceiveData.Servo[id].Current; }
            public int Temperature(byte id) { return ReceiveData.Servo[id].Temperature; }

            public int[] Position(params byte[] id)
            {
                int[] result = new int[id.Length];
                for (int i = 0; i < id.Length; i++) result[i] = ReceiveData.Servo[id[i]].Position;
                return result;
            }
            ...
            {
                int[] result = new int[id.Length];
                for (int i = 0; i < id.Length; i++) result[i] = ReceiveData.Servo[id[i]].Velocity;
                return result;
            }
            ...
            {
                int[] result = new int[id.Length];
                for (int i = 0; i < id.Length; i++) result[i] = ReceiveData.Servo[id[i]].Current;
                return result;
            }
            ...
            {
                int[] result = new int[id.Length];
                for (int i = 0; i < id.Length; i++) result[i] = ReceiveData.Servo[id[i]].Temperature;
                return result;
            }
            ...
            private long timeStamp = 0, timeStampOld = 0;
            public long TimeSpanMs = 4;
            ...
            private void SendData(byte id, byte[] sendData, byte instruction)
            {
                if (SerialPort.IsOpen)
                {
                    ...
                    if (timeStamp - timeStampOld < TimeSpanMs)
                    {
                        Thread.Sleep((int)(TimeSpanMs - (timeStamp - timeStampOld)));
                    }

                    timeStampOld = timeStamp;

                    SerialPort.Write(sendBytes, 0, length + 7);
                }
                else
                {
                    Console.WriteLine("Serial port is not open.");
            ...
            public void ChangeCurrentControlMode(params byte[] id)
            {
                ChangeControlMode(id, 0x00);
            }

            public void ChangeVelocityControlMode(params byte[] id)
            {
                ChangeControlMode(id, 0x01);
            }

            public void ChangePositionControlMode(params byte[] id)
            {
                ChangeControlMode(id, 0x03);
            }

            public void ChangeExtendedPositionControlMode(params byte[] id)
            {
                ChangeControlMode(id, 0x04);
            }

            public void ChangeCurrentBasedPositionControlMode(params byte[] id)
            {
        ...
        public class Leptrino : SerialBase
        {
            private ReceiveDataHandler ReceiveData = new ReceiveDataHandler();
            ...
            public short[] Force { get { return ReceiveData.Force; } }
            public short Fx { get { return ReceiveData.Force[0]; } }
            public short Fy { get { return ReceiveData.Force[1]; } }
            public short Fz { get { return ReceiveData.Force[2]; } }
            public short Mx { get { return ReceiveData.Force[3]; } }
            public short My { get { return ReceiveData.Force[4]; } }
            public short Mz { get { return ReceiveData.Force[5]; } }

            public float[] ForceLimit { get { return ReceiveData.ForceLimit; } }
            public float FxLimit { get { return ReceiveData.ForceLimit[0]; } }
            public float FyLimit { get { return ReceiveData.ForceLimit[1]; } }
            public float FzLimit { get { return ReceiveData.ForceLimit[2]; } }
            public float MxLimit { get { return ReceiveData.ForceLimit[3]; } }
            public float MyLimit { get { return ReceiveData.ForceLimit[4]; } }
            public float MzLimit { get { return ReceiveData.ForceLimit[5]; } }

            public string ModelName { get { return Encoding.ASCII.GetString(ReceiveData.ModelName); } }
            public string SerialNum { get { return Encoding.ASCII.GetString(ReceiveData.SerialNum); } }
            public string FirmVersion { get { return Encoding.ASCII.GetString(ReceiveData.FirmVersion); } }
            public string OutputRate { get { return Encoding.ASCII.GetString(ReceiveData.OutputRate); } }
            ...
            public void ConsoleWriteProductInfo()
            {
                ...
                Console.WriteLine("SerialNo : {0}", SerialNum);
            ...
            private class ReceiveDataHandler
            {
                ...
                public byte[] SerialNum = new byte[8];
        ...
        public class M5Stack : SerialBase
        {
            private ReceiveDataHandler ReceiveData = new ReceiveDataHandler();
            private static Header Header = new Header(0xFF, 0x7F);

            public M5Stack(int baudRate = 9600)
            {
                SetBaudRate(baudRate);
                SerialPort.Handshake = Handshake.None;
                SerialPort.DtrEnable = false;
